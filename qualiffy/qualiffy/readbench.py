...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--maxmononucreads', type=int, required=False, default=500, help='maximum number of reads to be evaluated for any one homopolymer run in the homopolymer run accuracy analysis. Use 0 to analyze all reads')
    ...
    parser.add_argument('--newflankrules', action='store_true', required=False, help='use new flank rules requiring at least 5bp of flanking seq outside HPs (no clipping yet)')
...
def read_config_data(args)->dict:
    ...
    if not configpath.exists():
        ...
        #template_res = importlib.resources.files("qualiffy").joinpath('benchconfig.txt')
