    ...
    with open(bedfile, "r") as bfh:
        ...
        while markerline:
            ...
            if current_scaffold != scaffold:
                ...
                if current_scaffold != "":
                    #phaseblockbedintervals = scaffold_phaseblock_intervals(timepoint, haplogprobs[-1], previousstate, intermarkercoords)
                    path = [0] * timepoint
                    lastlogprobs = haplogprobs.pop()
                    if lastlogprobs[0] > lastlogprobs[1]:
                        path[timepoint-1] = 0
                        #logger.debug("Assigning haplotype " + str(path[timepoint-1]) + " to last position of " + scaffnames[current_scaffold] + " log probs are " + str(lastlogprobs[0]) + "/" + str(lastlogprobs[1]))
                    else:
                        path[timepoint-1] = 1
                        #logger.debug("Assigning haplotype " + str(path[timepoint-1]) + " to last position of " + scaffnames[current_scaffold] + " log probs are " + str(lastlogprobs[0]) + "/" + str(lastlogprobs[1]))
                    ...
                    for currenttime in range(timepoint):
                        ...
                        #print(scaffnames[current_scaffold] + "\t" + str(blockstart) + "\t" + str(blockend) + "\t" + currenthap)
                        fh.write(scaffnames[current_scaffold] + "\t" + str(blockstart) + "\t" + str(blockend) + "\t" + currenthap + "\t0\t+\t" + str(blockstart) + "\t" + str(blockend) + "\t" + bedcolor + "\n")
                    num_hapmers = num_hapmers + timepoint
                    timepoint = 0
            ...
            markerline = bfh.readline()
    #print("Read in " + str(num_hapmers) + " hapmer locations")
    mfh.close()
    pfh.close()
    sfh.close()
...
def find_haplotype_names_in_hapmer_bedfile(bedfile:str):
    ...
    if hap1 is not None and hap2 is not None:
        if pmathap.match(hap1):
            ...
            logger.info("Found " + hap2 + " and " + hap1 + " as haplotypes.")
            print("Found " + hap2 + " and " + hap1 + " as haplotypes.")
            return [hap2, hap1]
    else:
...
def scaffold_phaseblock_intervals(timepoint:int, lasthapprobs:list, previousstate:list, intermarkercoords:list):
    prevstatelength = len(previousstate)
    intermarkerlength = len(intermarkercoords)
    #print("In scaffold_phaseblock_intervals with " + str(prevstatelength) + " previous states and " + str(intermarkerlength) + " inter marker coords and " + str(timepoint) + " value of timepoint")
